...
<BODY>
<br><br><FORM action="http://localhost/control/html/resultado3.php" method="post"> <!--Declaro un formulario que enlaza con resultado3 y envio los datos por el metodo post-->
<?php 
...
//fetch_assoc es un metodo del la instancia linkidi del objeto mysql que devuelve una fila mientras haya
	...
	echo "Poblacion  Ciudad<INPUT type=text name='poblacion' value='".$row["Population"]."'><br>";// creo un campo imput tipo texto de nombre population con el valor que recojo del campo population que se encuentra en la variable row en ese momento como parte del registro que devuelve la consulta este campo es editable
	...
	echo "<INPUT type=hidden name=CountryCode value=".$row["CountryCode"]."><br>"; // creo un campo imput tipo hidden oculto de nombre CountryCode con el valor que recojo del campo CountryCode que se encuentra en la variable row en ese momento como parte del registro que devuelve la consulta este campo no es editable puesto que esta oculto
...
?>
<br><input type="submit" name='boton' value="Actualizar"> <!--declaro un boton tipo submit para enviar datos y le doy el valor Actualizar-->
<input type="submit"  name='boton' value="Borrar"><!--declaro un boton tipo submit para enviar datos y le doy el valor Borrar aqui el nombre  boton a los dos elementos y es porque 
como solo podemos escoger una opcion y siempre recogeremos en funcion del nombre del elemento podremos hacer condiciones en funcion de lo que valga el boton, por ejemplo si el boton vale
actualizar hago una cosa y si vale borrar hago otra-->
