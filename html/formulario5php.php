...
<BODY>
<br><br><FORM action="http://localhost/control/html/resultado4.php" method="post"> <!--Declaro un formulario que enlaza con resultado4 y envio los datos por el metodo post-->
<?php 
...
//fetch_assoc es un metodo del la instancia linkidi del objeto mysql que devuelve una fila mientras haya
	...
	echo "<OPTION name='pais' VALUE='".$row2["Code"]."' selected>".$row2["Name"]."</option>";//imprimimos parte del select donde el valor del select sera el campo code y ademas  en option marcamos selected por que es la opcion seleccionada y la que se enviara a otros archivos en caso de envios
	...
	echo "<OPTION name='pais' VALUE='".$row2["Code"]."'>".$row2["Name"]."</option>";// monto otro select igual, pero no marco el seleccionado, puesto que no coinciden
...
<input type="submit"  name=boton value="Borrar"><!--declaro un boton tipo submit para enviar datos y le doy el valor Borrar-->
<input type="submit"  name=boton value="Insertar"> <!--declaro un boton tipo submit para enviar datos y le doy el valor Insertar aqui el nombre  boton a los dos elementos y es porque 
