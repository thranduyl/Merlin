...
// creo la variable consulta2 y le meto un select pero esta vez de otra tabla 
...
while ($row2 = $resultados2->fetch_assoc()){//llamo al metodo fetch_asoc del objeto mysqli que devuelve registros. Mientras haya un registro lo asigno a la variable row
//mientras que queden filas en query results asignamos a $row como si es fila o lo que sea
//fetch_assoc es un metodo del la instancia linkidi del objeto mysql que devuelve una fila mientras haya
	//es muy importante el uso de las comillas sobre todo al crear parametros o consultas por que o la consulta sale mal o el formulario manda mal el dato y no coje el dato completo en el caso de que haya espacios ademas los nombres del elemento de formulario deben ir entre comillas
	...
	echo "<OPTION VALUE='".$row2["Name"]."' selected>".$row2["Name"]."</option>";//imprimimos parte del select donde el valor del select sera el campo code y ademas  en option marcamos selected por que es la opcion seleccionada y la que se enviara a otros archivos en caso de envios
	...
	echo "<OPTION VALUE='".$row2["Name"]."'>".$row2["Name"]."</option>";// monto otro select igual, pero no marco el seleccionado, puesto que no coinciden
